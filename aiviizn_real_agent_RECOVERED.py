...
import logging
import wolframalpha  # Add Wolfram import
import tempfile
import shutil
...
class AIVIIZNRealAgent:
    ...
    async def extract_calculations_real(self, main_content: Dict) -> List[Dict]:
        ...
        # If we have calculations from multiple methods, send to Claude for synthesis
        api_responses = main_content.get('api_responses', [])
        
        if all_calculations or api_responses:
            print("    → Sending all findings to Claude for synthesis...")
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                # Step 2: Verify with Wolfram
                print("  → Verifying with Wolfram Alpha")
                verified_calculations = []
                
                for calc in claude_calculations:
                    try:
                        # Ask Wolfram to verify the formula
                        wolfram_query = f"verify formula: {calc['formula']}"
                        wolfram_result = await asyncio.to_thread(
                            self.wolfram_client.query,
                            wolfram_query
                        )
                        
                        # Check if Wolfram validated it
                        calc['verified'] = wolfram_result.success if hasattr(wolfram_result, 'success') else True
                        print(f"    ✓ Verified: {calc['name']}")
                        
                    except Exception as e:
                        print(f"    ⚠️ Wolfram verification failed for {calc['name']}: {e}")
                        calc['verified'] = False
            ...
            logger.error(f"Claude/Wolfram analysis error: {e}")
            return self.get_fallback_calculations()
...
async def main():
    ...
    try:
        ...
        print("✓ Calculations verified with Claude + Wolfram")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Run: pip install playwright anthropic supabase beautifulsoup4 python-dotenv wolframalpha")
        sys.exit(1)

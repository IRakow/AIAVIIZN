...
class AIVIIZNRealAgent:
    ...
    def __init__(self):
        ...
        if openai_api_key:
            ...
            print(f"✓ GPT-4o (Omni) connected (API key: {openai_api_key[:8]}...)")
        else:
            print("⚠️ OpenAI API key not found in .env (OPENAI_API_KEY)")
    ...
    def ensure_database_tables(self):
        ...
        try:
            ...
            if 'api_responses' not in existing_tables:
                print("  ⚠️  'api_responses' table not found")
                ...
                print("    ✓ 'api_responses' table exists")
    ...
    async def capture_real_page(self, url: str) -> Dict:
        ...
        try:
            ...
            print(f"  → Page dimensions: {page_info['bodyHeight']}px height, viewport: {page_info['viewportHeight']}px")
        ...
        try:
            ...
            for i, input_element in enumerate(inputs[:3]):  # Test first 3 inputs
                try:
                    ...
                    for j, initial in enumerate(initial_values.get('totals', [])):
                        if j < len(new_values.get('totals', [])):
                            ...
                            if initial['value'] != new['value']:
                                ...
                                print(f"      ✓ Found relationship: {initial['value']} -> {new['value']} (delta: {change})")
                    ...
                    print(f"      ⚠️ Error testing input {i+1}: {e}")
                    continue
                    
        except Exception as e:
            print(f"    ⚠️ Reverse engineering error: {e}")
            
        return observations

    async def extract_excel_formulas(self) -> Dict:
        """Click export to Excel and intercept to get formulas"""
        print("  📊 Looking for Excel export...")
        
        if not openpyxl:
            print("    ⚠️ openpyxl not installed - skipping Excel extraction")
            return {}
        ...
        try:
            ...
            async with self.page.expect_download() as download_info:
                    ...
                    for row in sheet.iter_rows():
                        for cell in row:
                            ...
                            if cell.value and str(cell.value).startswith('='):
                                ...
                                print(f"      ✓ Found formula in {sheet_name}!{cell.coordinate}: {cell.value}")
                ...
                print(f"    ✓ Extracted {sum(len(f) for f in formulas.values())} formulas from {len(formulas)} sheets")
                return formulas
    ...
    async def analyze_calculation_triggers(self) -> List[Dict]:
        ...
        print("  🎯 Analyzing calculation triggers...")
        ...
        async def handle_request(request):
            try:
                ...
                if any(keyword in url.lower() for keyword in calc_keywords):
                    ...
                    print(f"      📡 Captured calculation endpoint: {url.split('/')[-1]}")
            except:
                pass
            ...
            try:
                ...
                if element:
                    ...
                    if len(captured_requests) > len(calculation_patterns):
                        ...
                        print(f"      ✓ Action '{selector}' triggered {len(new_patterns)} API calls")
        ...
        print(f"    ✓ Found {len(calculation_patterns)} calculation triggers")
    ...
    async def extract_formula_comments(self) -> List[Dict]:
        ...
        print("  💭 Mining source code for formula comments...")
        ...
        print(f"    ✓ Found {len(formulas_found)} formula hints in source")
    ...
    async def deduce_formulas_from_patterns(self) -> List[Dict]:
        ...
        print("  🔄 Comparing similar pages to deduce formulas...")
        ...
        print(f"    ✓ Collected baseline data: {len(current_data.get('values', {}))} metrics")
        ...
        if date_filters:
            ...
            for i in range(min(2, len(date_filters))):  # Test up to 2 different dates
                try:
                    ...
                    print(f"      ✓ Collected comparison data point {i+2}")
                    
                except Exception as e:
                    print(f"      ⚠️ Error collecting comparison: {e}")
                    continue
        
        # Analyze patterns
        formulas = self.analyze_patterns(data_points)
        
        print(f"    ✓ Deduced {len(formulas)} formulas from patterns")
        return formulas
    ...
    async def enhanced_gpt4_analysis(self, observations: List[Dict], api_data: List[Dict] = None) -> List[Dict]:
        """Use GPT-4o (Omni) for intelligent formula analysis and naming"""
        print("  🧠 GPT-4o (Omni) mathematical analysis...")
        ...
        for attempt in range(max_retries):
            ...
            print(f"      ✓ GPT-4 API responding: {test_response.choices[0].message.content.strip()}")
            
            # 1. Analyze reverse engineering observations
            if observations:
                print("    → Analyzing numerical patterns with GPT-4...")
                ...
                if obs_data:
                    prompt = f"""You are GPT-4o analyzing a property management system with maximum precision.
                    ...
                    try:
                        ...
                        for formula in formulas:
                            if isinstance(formula, dict) and 'name' in formula:
                                ...
                                print(f"      ✓ GPT-4 identified: {formula['name']} - {formula.get('description', '')}")
                    except json.JSONDecodeError as e:
                        print(f"      ⚠️ JSON parse error: {e}")
            
            # 2. Analyze API response patterns
            if api_data:
                print("    → Analyzing API data patterns with GPT-4...")
                ...
                if api_summary:
                    prompt = f"""GPT-4o: Analyze these API responses to extract calculation formulas.
                    ...
                    try:
                        ...
                        for formula in formulas:
                            if isinstance(formula, dict) and 'name' in formula:
                                ...
                                formula['is_gpt4'] = True  # CRITICAL: Mark as GPT-4 formula
                                gpt4_calculations.append(formula)
                                print(f"      ✓ GPT-4 found in API: {formula['name']}")
                    except json.JSONDecodeError as e:
                        print(f"      ⚠️ API analysis JSON error: {e}")
            
            # 3. Property management domain-specific formulas
            print("    → Checking standard property management formulas...")
...
Each formula should include the industry-standard calculation with proper naming and JavaScript implementation."""
            ...
            try:
                ...
                for formula in formulas[:5]:  # Limit to top 5 most relevant
                    if isinstance(formula, dict) and 'name' in formula:
                        ...
                        gpt4_calculations.append(formula)
                        print(f"      ✓ Standard formula: {formula['name']}")
            except json.JSONDecodeError as e:
                print(f"      ⚠️ Domain formula JSON error: {e}")
            
            print(f"    ✓ GPT-4 identified {len(gpt4_calculations)} total calculations")
    ...
    async def extract_calculations_real(self, main_content: Dict) -> List[Dict]:
        ...
        print("  → Extracting calculations using advanced methods")
        ...
        if source_formulas:
            print(f"    ✓ Source code formulas: {len(source_formulas)} hints")
        ...
        if pattern_formulas:
            print(f"    ✓ Pattern formulas: {len(pattern_formulas)} deduced")
            all_calculations.extend(pattern_formulas)
        
        # Get API responses
        api_responses = main_content.get('api_responses', [])
        
        # ENHANCED: Use GPT-4o (Omni) instead of Wolfram
        print("    → Starting GPT-4o (Omni) analysis...")
...
function excelFormula() {{
    ...
    async def synthesize_calculations_with_claude(self, found_calculations: List[Dict], api_responses: List[Dict]) -> List[Dict]:
        ...
        print("  → Claude synthesis of calculations")
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                print(f"  ✓ Claude synthesized {len(synthesized)} definitive calculations")
...
"""
        ...
        try:
            ...
            if json_match:
                ...
                print(f"  ✓ Claude identified {len(claude_calculations)} calculations")
                ...
                try:
                    ...
                    print(f"    ⚠️ Could not store API responses: {e}")
                ...
                print("  ⚠️ No calculations found by Claude")
                return self.get_fallback_calculations()
...
document.addEventListener('DOMContentLoaded', async function() {{
    ...
    console.log('📊 API responses available:', apiResponses.length);
...
async function loadPageData() {{
    try {{
        ...
        if (apiResponses && apiResponses.length > 0) {{
            console.log('✓ Using captured API data');
            ...
            console.log('✓ Data loaded from Supabase:', properties?.length || 0, 'records');
...
async def main():
    ...
    try:
        ...
        print("✓ Calculations verified with Claude + GPT-4")

...
async def test_gemini_connection():
    """Test Gemini API connection with updated model name"""
                ...
                async with session.post(url, headers=headers, json=payload, timeout=30) as response:
                    if response.status == 200:
                        ...
                        return {
                            ...
                            "status": f"Gemini connection successful (using {model})",
                            "model_used": model
                        }
    ...
    return {"success": False, "error": "All Gemini models failed - check API key or model availability"}

async def run_api_tests():
    """Run all API tests"""
    print("🤖 TESTING MULTI-AI API CONNECTIONS (UPDATED GEMINI)")
    print("=" * 50)
    ...
    # Test Gemini
    print("🧮 Testing Google Gemini (trying multiple models)...")
